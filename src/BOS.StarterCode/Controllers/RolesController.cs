...
namespace BOS.StarterCode.Controllers
{
    ...
    public class RolesController : Controller
    {
        ...
        public RolesController(IConfiguration configuration, IHttpContextAccessor contextAccessor)
        {
        ...
        public async Task<IActionResult> EditRole(string roleId)
        {
            try
            {
                if (!string.IsNullOrEmpty(roleId)) //Looking for a non-null roleId
                {
                    ...
                    else if (roleInfo != null && roleInfo.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                    {
