...
namespace BOS.StarterCode.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        private IAuthClient _bosAuthClient;
        private IEmailClient _bosEmailClient;
        private Logger Logger;

        public UsersController(IConfiguration configuration, IHttpContextAccessor contextAccessor)
        {
        ...
        public async Task<IActionResult> AddNewUser()
        {
            try
            {
                ...
                else if (availableRoles != null && availableRoles.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                {
        ...
        public async Task<IActionResult> EditUser(string userId)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(userId)) //Checking for a non-null, non-empty userId
                {
                    ...
                    else if (userInfo != null && userInfo.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                    {
                        return RedirectToAction("SignOut", "Auth");//Token Expired
                    }
                            ...
                            if (roleList != null && roleList.Count > 0) //Non-null role List and with at least one record
                            {
                                ...
                                if (result != null && result.IsSuccessStatusCode)
                                {
                                    ...
                                    if (extendUserResponse != null && extendUserResponse.IsSuccessStatusCode)
                                    {
                                        ...
                                        if (roleResponse != null && roleResponse.IsSuccessStatusCode)
                                        {
                                            ...
                                            if (isEmailToSend)
                                            {
                                                ...
                                                if (slugResponse != null && slugResponse.IsSuccessStatusCode)
                                                {
                                                    ...
                                                    else if (slugResponse != null && slugResponse.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                                                    {
                                                    ...
                                                    else if (spResponse != null && spResponse.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                                                    {
                                                        return "Token Expired, Please try again";//Token Expired
                                                    }
                                                    else
                                                    ...
                                                    emailObj.Substitutions.Add(new Substitution { Key = "applicationUrl", Value = baseUrl + "/Password/Reset?slug=" + slug.Value + "&set=true" });
                                        ...
                                        else if (roleResponse != null && roleResponse.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                                        {
                                            return "Token Expired, Please try again";//Token Expired
                                        }
                                    }
                                ...
                                else if (result != null && result.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                                {
                                    return "Token Expired, Please try again";//Token Expired
                                }
                                else
        ...
        private async Task<dynamic> GetPageData()
        {
            try
            {
                ...
                if (HttpContext != null && HttpContext.Session != null)
                {
                    ...
                    else if (userList != null && userList.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                    {
                        //return "Token Expired, Please try again";//Token Expired
                    }
